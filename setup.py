    ...
    def run(self):
        ...
        if platform.system() == "Windows":
            ...
            if cmake_version < '3.12.0':
                raise RuntimeError("CMake >= 3.12.1 is required on Windows")
...
class GoogleTestCommand(TestCommand):
    ...
    def run(self):
        ...
        subprocess.call(['make cimod_test'],
                        cwd=os.path.join('build',
                                         self.distutils_dir_name('temp')),
                        shell=True)
        subprocess.call(['./tests/cimod_test'],
                        cwd=os.path.join('build',
                                         self.distutils_dir_name('temp')),
                        shell=True)
...
setup(
    ...
    install_requires=['numpy >= 1.18.4, !=1.21.0, !=1.21.1', 'dimod >= 0.9.1, < 0.12.0', 'scipy'],
